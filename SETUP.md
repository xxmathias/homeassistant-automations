...
|------|-----------|-------------|-------|
| Wohnzimmer | 15 min → dim 50% → 30s → off | 1 min → off | Skips dim if light already off. ANY manual touch (on, off or brightness, desk lamp included) sets `input_boolean.wohnzimmer_keep_on`, which pauses every automatic turn-on AND turn-off until an automation leaves the room fully dark or presence is off 2h |
| Kitchen | 4 min (2+2) → off | 2 min → off | `aus` trigger has 2min `for:`, day adds 2min delay. Both kuchenlampe *and* billy_tischleuchte |
...
- **Manual-touch detection via `context.user_id`** — `wohnzimmer_manual_override` and `durchgangszimmer_manual_override` treat a light change as manual only when `trigger.to_state.context.user_id is not none`. Frontend/app/voice calls carry a user_id; automation `light.turn_on` calls do not, so our own presence/daylight actions never trip the hold. The Tuya remote is deliberately NOT manual: its blueprint runs in an automation context, so no button press engages a hold. Confirmed 2026-09-11. Brightness and hand-set states come from the app, the frontend or voice.
- **`wohnzimmer_keep_on` must be checked everywhere wohnzimmer can be turned on or off** — `wohnzimmer_on_presence` (the desklamp block, both presence-on branches and both presence-off branches, re-checked after the delay), `stuck_lights_cleanup` (fires at 10 min), `daylight_lights_on` (wohnzimmer branch) AND `daylight_lights_off` (>50 lux, incl. the desklamp sub-branch). Miss one and the hold leaks. The hold engages on any user-initiated change with a non-null `context.user_id`, off included, and is released when an automation leaves all 3 lights + the desk lamp off, or presence is off 2h.
...
- **`light.billy_tischleuchte` mirrors `light.kuchenlampe`** — it lives in the kitchen area (`kuche`) and is targeted by entity_id everywhere the kuchenlampe is (presence on/off, both pathway automations, night pathway, daylight-on, stuck-lights). Same levels: 100% day / 60% night, 50% day-pathway, 20% night-pathway. Area-targeted actions (`daylight_lights_off`, `script.bedtime_mode`, `script.turn_off_all`) pick it up automatically via the area. Add it to any *new* kitchen entity_id target too, or it gets stranded on.
...
- **`switch.turn_off` sweeps over `kuche` also hit the Kitchen Adaptive Lighting switches** — the area has no switch-driven lamp, so `daylight_lights_off`, `script.turn_off_all` (which the Tuya remote now calls) turning switches off there only ever touches the Adaptive Lighting controls. Long-standing behaviour, left as-is; worth knowing if AL for the kitchen ever seems to disable itself.
