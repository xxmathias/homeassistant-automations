...
|------|-----------|-------------|-------|
| Wohnzimmer | 4 min → dim 50% → flash → off | 2 min → off | Skips dim if light already off |
| Kitchen | 4 min (2+2) → dim 50% → flash → off | 2 min → off | `aus` trigger has 2min `for:`, day adds 2min delay |
| Durchgangszimmer | 4 min → dim 50% → off | — | Template trigger: both sensors off for 4 min. Night handled by night_pathway_mode |
