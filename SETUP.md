...
- **`light.billy_tischleuchte` mirrors `light.kuchenlampe`** — it lives in the kueche area and is targeted by entity_id everywhere the kuchenlampe is (presence on/off, both pathway automations, night pathway, daylight-on, stuck-lights). Same levels: 100% day / 60% night, 60% day-pathway, 30% night-pathway, 50% dim warning. Area-targeted actions (`daylight_lights_off`, `script.bedtime_mode`, `script.turn_off_all`) pick it up automatically via the area. Add it to any *new* kitchen entity_id target too, or it gets stranded on.
...
- **Area ids are `kueche` and `entrance`** — both confirmed against HA (2026-08-25). `kuche` (one 'e') was a dead id in `daylight_lights_off`, `tuya_4button_remote`, `bedtime_mode` and `turn_off_all`, so those never touched the kitchen; fixed. Do NOT "correct" `entrance` to `eingang` — `entrance` is the real id there.
- **`kueche` belongs in `light.turn_off` area sweeps but NOT `switch.turn_off` ones** — the kitchen has no switch-driven lamp; its only switches are the Kitchen Adaptive Lighting controls (and praesenzsensor config). Sweeping switches over the area would silently disable Adaptive Lighting. While the area id was misspelled `kuche` this was harmless; once fixed it became live, so `daylight_lights_off`, `script.turn_off_all` and `tuya_4button_remote` list only the areas that actually have lamp switches. Same reasoning as the explicit wohnzimmer switch list that protects the Yamaha boxes.
